...
rarefyDataset <- function(mydata, sampletype= NA, depth, seed= 1234){
  if(!is.na(sampletype)){
    mydata <- subset_samples(mydata, type == sampletype)
  }
...
```{r}
#1) top 50 i indløb
top50.ww <- prune_taxa(names(sort(taxa_sums(WWDatasets[["otu94"]]),TRUE)[1:50]), 
                      datasets[["otu94"]]) 
sample_data(top50.ww)$Desc <- sort(with(sample_data(top50.ww), paste(type, plant_name)), decreasing = TRUE) 
top50ww.heatmap <- plot_heatmap(top50.ww, method = "NMDS", distance = "bray", 
                                sample.label = "Desc", taxa.label = "Genus", 
                                low="#66CCFF", high="#000033")
top50ww.heatmap


#2) top 50 arter i aktivt slam?
top50.as <- prune_taxa(names(sort(taxa_sums(ASDatasets[["otu94"]]),TRUE)[1:50]), 
                      datasets[["otu94"]]) 
sample_data(top50.as)$Desc <- sort(with(sample_data(top50.as), paste(type, plant_name)), decreasing = TRUE) 
top50as.heatmap <- plot_heatmap(top50.as,  
                                sample.label = "Desc", taxa.label = "Genus", 
                                low="#66CCFF", high="#000033")
top50as.heatmap
